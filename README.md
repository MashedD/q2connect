...
`hide_wallfly` defaults to `true`. While enabled, players named exactly `WallFly[BZZZ]` or `Ghost[BOOO!]` are hidden from the player details and excluded from each server's player count and the total player count. Set it to `false` in `q2connect.json` to show and count them.
